...
class ActorCritic(nn.Module):
    ...
    def _normalize_observation(self, obs):
        ...
        clean_var = torch.nan_to_num(self.obs_var, nan=1.0).clamp(min=1e-4)
    ...
    def _normalize_critic_observation(self, obs):
        ...
        clean_var = torch.nan_to_num(self.critic_obs_var, nan=1.0).clamp(min=1e-4)
    ...
    def _merge_observation_stats(observations, mean, var, count):
        ...
        safe_var = torch.nan_to_num(var, nan=1.0).clamp(min=1e-4)
        ...
        new_var = torch.nan_to_num((current_m2 + batch_m2 + correction) / total_count, nan=1.0).clamp(min=1e-4)
    ...
    def _distribution(self, obs):
        ...
        mean = torch.nan_to_num(raw_mean, nan=0.0)
        log_std = torch.clamp(
            self.actor_log_std,
    ...
    def evaluate(self, obs, action, critic_obs=None):
        ...
        value = torch.nan_to_num(raw_value, nan=0.0)
        return log_prob, entropy, value

    def get_value(self, obs, critic_obs=None):
        critic_obs = obs if critic_obs is None else critic_obs
        raw_value = self.critic(self._normalize_critic_observation(critic_obs)).squeeze(-1)
        return torch.nan_to_num(raw_value, nan=0.0)
