...
def compute_gae(
    ...
    Args:
        rewards: ``[time, env]`` rewards.
        values: value estimates for the same states, ``[time, env]``.
        terminated: terminal flags, ``[time, env]``.
        next_value: value estimate for the state after the rollout, ``[env]``.
        truncated: optional time-limit flags, ``[time, env]``.
        truncation_values: values of the terminal observations for time-limit
            transitions, ``[time, env]``. Required when any flag is true.
    """
