# ruff: noqa: E402
"""Real-time native MuJoCo preview for a trained Go1 policy.

The exact evaluator uses MJX.  Native MuJoCo is used here only for responsive
visual inspection on Windows, where JAX/MJX normally runs on the CPU.
"""
    ...
    *,
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        while viewer.is_running():
            ...
            last_action = action.squeeze(0).numpy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Real-time native MuJoCo policy preview")
